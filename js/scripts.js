...
// if sell_holdings and sell_rate exist
	...
	function insertValues() {
		for (var prop in form) {
			...
			//log(`${prop} = ${form[prop]}`);
				...
				if( !exclude.includes(prop) && checkInputFormType() == 0) {  
					document.getElementById(`${prop}`).value  = formatOutput(prop, form[prop]);
					//log(`${prop} = ${form[prop]}`);
				}
				// for input type=number
				if( !exclude.includes(prop) && checkInputFormType() == 1) {  
					document.getElementById(`${prop}`).value = inputOutputNumber(form[prop]);
				}
			}
		}
	}
  	...
  	function changeFee(reset=0) {
	  		...
	  		if(checkInputFormType() == 0) {
	  			...
	  			if(reset == 0){
	  				varFee.forEach(function(element) {
	  					//log(element+": reset = 0");
			  			document.getElementById(element).value  = inputOutputNumber(0);
	  				});
  				}
	  			if(reset == 1){
	  				varFee.forEach(function(element) {
	  					//log(element+": reset = 1");
			  			document.getElementById(element).value  = inputOutputNumber('');
	  				});
	  			}
	  		}
	...
	//*************************//
  	...
  	function checkInputValue() {	
			if( document.getElementById("buy_fee").value == 0 && pageVariables.inputs[2].isEmpty == 0) {
				changeFee();
			}
	...
	//*************************//
  	...
  	//*************************//
  	// Check form input type, act accordingly for text vs number
  	// returns 1 for input type = number
  	function checkInputFormType() {
  		let value = 0;
  		let count = 0;
  		pageVariables.inputs.forEach(function(layer){
  			if(layer.element.getAttribute("type") == "number"){
  				count++;
  			}
  		});
  		if(count >= 7){
  	...
  	function inputOutputNumber(input) {
  		log(input);
